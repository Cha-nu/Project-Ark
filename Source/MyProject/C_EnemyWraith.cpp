...
void AC_EnemyWraith::Tick(float DeltaTime)
{
    ...
    // [Step 2] 회전 로직 (공격 여부와 상관없이 플레이어를 항상 바라보게 함)
    // 단, 사거리 근처에 있을 때만 정밀하게 바라보도록 설정
    if (DistanceToPlayer <= AttackRange * 1.5f) 
    {
        // 이동 방향으로 자동 회전하는 옵션을 일시적으로 끄거나 무시
    ...
    if (DistanceToPlayer <= AttackRange) 
    {
        ...
        // 공격 중이 아닐 때만 공격 시도 (회전은 위에서 이미 처리 중)
        if (!bIsAttacking)
        {
            Attack();
        }
...
float AC_EnemyWraith::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
    ...
    if (HitMontage && DamageCauser)
    {
        ...
        GetWorldTimerManager().SetTimer(HitStunTimerHandle, this, &AC_EnemyWraith::ResetMovement, HitStunDuration, false);
...
void AC_EnemyWraith::ResetMovement()
{
...
void AC_EnemyWraith::Die()
{
    ...
    if (DeathMontage && LastAttacker)
    {
        ...
        FName SectionName = FName("DeathFor"); // 기본값
